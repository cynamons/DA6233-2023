...
Use `fill` to change the inside of the bars.\
Use `alpha` to change the transparency of the bars.\

```{r}
ggplot(mpg, aes(x = hwy)) +
  geom_histogram(bins = 15, color = "white", fill='#CA1C1E', alpha = 0.75)
```
...
```{r}
mean_hwy = mpg %>% summarize(mean(hwy)) %>% as.numeric()
mpg %>% 
  group_by(class) %>% 
  summarize(count = n()) %>% 
  ggplot(aes(x = class, y = count)) +
  geom_col()+
  geom_hline( yintercept = mean_hwy, color='blue')
```
