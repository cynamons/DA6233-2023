...
.default-blk code.sourceCode > span { /* plain text/variables */
  color: gold
}
.default-blk .cn { /* constants */
  color: lightgrey
}
.default-blk .co { /* comments */
  color: indigo
}
.default-blk .fu { /* function name */
  color: blue
}
.default-blk .f1 { /* number values */
  color: red
}
.default-blk .ot { /* equal sign */
  color: white
}
.default-blk .sc { /* operators */
  color: white
}
.default-blk .at { /* column names */
  color: lightgrey
}
.default-blk .st { /* string in quotes */
  color: green
}
```

```{r, echo=FALSE}
knitr::opts_chunk$set(class.source = "default-blk")
```
...
```{r}
#| echo: false
cleanrmd::use_cleanrmd(name = "sakura-vader")
```
...
```{r}
p1 = economics %>% 
  mutate(year = lubridate::year(date)) %>%  #my common comments
  filter(year >= 2010) %>% 
  group_by(year) %>% 
...
```{r}
 economics %>% 
  mutate(unemploy_per = unemploy / pop) %>% 
  ggplot(aes(date, unemploy_per)) +
  annotation_raster(readPNG(here::here("Data","UTSA_Logo_Website_UTSA_Logo_Stacked_Center.png")),
